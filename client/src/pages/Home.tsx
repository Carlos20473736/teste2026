/*
 * MONETAG AUTO BOT - Versão SEM IA (Random Humanizado)
 * 
 * Funcionalidades:
 * 1. Botão de iniciar/parar manual
 * 2. Timings fixos com variação aleatória (nunca 100% igual)
 * 3. Countdown de 1h quando bater metas (impressões + cliques)
 * 4. Histórico completo salvo no localStorage
 * 5. Integração com IFTTT
 */
...
class ECPMOptimizer {
  ...
  // Valores BASE fixos + jitter aleatório para cada sessão
  ...
  constructor() {
    // Gera variações aleatórias em cima dos valores base
    // Cada instância (sessão) terá timings ligeiramente diferentes
    this.BASE_COOLDOWN = this.randomize(8000, 0.25);       // 6000-10000ms
    this.MIN_COOLDOWN = this.randomize(5000, 0.2);          // 4000-6000ms
    this.MAX_COOLDOWN = this.randomize(45000, 0.2);         // 36000-54000ms
    this.WARM_UP_COOLDOWN = this.randomize(12000, 0.25);    // 9000-15000ms
    this.INITIAL_DELAY = this.randomize(3000, 0.3);         // 2100-3900ms
    this.BACKOFF_MULTIPLIER = 1.4 + Math.random() * 0.4;   // 1.4-1.8
    this.SUCCESS_REDUCTION = 0.8 + Math.random() * 0.1;    // 0.80-0.90
    this.JITTER_RANGE = this.randomize(2000, 0.3);          // 1400-2600ms
    this.WARM_UP_ADS = Math.floor(2 + Math.random() * 3);  // 2-4
    this.OPTIMAL_SESSION_PACE = this.randomize(25, 0.3);    // 17-32s
    this.loadState();
  }

  /** Gera um valor base ± percentage de variação */
  ...
  private loadState() {
    try {
      const saved = sessionStorage.getItem('ecpm_optimizer');
      if (saved) {
  ...
  private saveState() {
    try {
      sessionStorage.setItem('ecpm_optimizer', JSON.stringify({
  ...
  recordAdResult(success: boolean, durationMs: number) {
    ...
    this.saveState();
    console.log(`[eCPM] Ad #${this.totalAdsShown} ${success ? 'OK' : 'FAIL'} | ` +
...
// ===== CONFIGURAÇÃO =====
const ZONE_ID = "11156884";
const DOMAIN = "libtl.com";
const SDK_GLOBAL = `show_${ZONE_ID}`; // show_11156884
...
const MAX_CLICKS = 2;
const RESET_DURATION_SECONDS = 3600; // 1 hora de countdown
...
function getConfig() {
  ...
  const ymid = params.get('ymid') || hashParams.get('ymid') || localStorage.getItem('monetag_ymid') || generateRandomYmid();
  const telegramId = params.get('tgid') || hashParams.get('tgid') || localStorage.getItem('monetag_tgid') || generateRandomTelegramId();
  const oaid = getRealOaid();

  localStorage.setItem('monetag_ymid', ymid);
  localStorage.setItem('monetag_tgid', telegramId);

  return { ymid, oaid, telegramId };
}
...
function buildParams(config: { ymid: string; oaid: string; telegramId: string }) {
...
async function fetchAds(config: { ymid: string; oaid: string; telegramId: string }) {
  await waitForRealOaid();
  const params = buildParams(config);
  const url = `https://e8ys.com/500/${ZONE_ID}?${params.toString()}`;
...
// Cada sessão gera valores ligeiramente diferentes para simular comportamento humano variável
function generateSessionTimings() {
  ...
  return {
    delayBetweenCyclesMin: rand(2000, 0.4),   // 1200-2800ms
    delayBetweenCyclesMax: rand(6000, 0.35),   // 3900-8100ms
    viewTimeBase: rand(15000, 0.3),            // 10500-19500ms
    viewTimeVariance: rand(5000, 0.4),         // 3000-7000ms
    clickDelayMin: rand(1500, 0.35),           // 975-2025ms
    clickDelayMax: rand(4000, 0.3),            // 2800-5200ms
    resolveDelayMin: rand(3000, 0.3),          // 2100-3900ms
    resolveDelayMax: rand(7000, 0.3),          // 4900-9100ms
    // Ciclos em que o clique vai acontecer (aleatório entre 1 e MAX_IMPRESSIONS)
    clickCycles: generateClickCycles(),
    // Chance de fazer uma pausa extra (simulando distração)
    pauseChance: 0.1 + Math.random() * 0.15,  // 10-25%
    pauseDuration: rand(8000, 0.4),            // 4800-11200ms
  };
}

function generateClickCycles(): number[] {
  // Gera 2 ciclos aleatórios para os cliques, distribuídos ao longo das impressões
  const cycles: number[] = [];
  const firstClick = Math.floor(2 + Math.random() * 8); // ciclo 2-9
  const secondClick = Math.floor(firstClick + 3 + Math.random() * 8); // 3-10 ciclos depois
...
interface HistoryEntry {
  ...
  clicks: number;
  revenue: number;
  duration: number; // ms que a sessão durou
  ymid: string;
}

function loadHistory(): HistoryEntry[] {
  try {
    const saved = localStorage.getItem('bot_session_history');
...
function saveHistoryEntry(entry: HistoryEntry) {
  try {
    ...
    localStorage.setItem('bot_session_history', JSON.stringify(trimmed));
  } catch {}
}
...
export default function Home() {
  ...
  const [iftttWebhook, setIftttWebhook] = useState(() => {
    try { return localStorage.getItem('ifttt_webhook') || ''; } catch { return ''; }
  });
  ...
  const optimizerRef = useRef(new ECPMOptimizer());
  ...
  const [revenue, setRevenue] = useState<number>(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('local_stats_bot') || '{}');
      return saved.revenue || 0;
    } catch { return 0; }
  });
  const [ecpmValue, setEcpmValue] = useState(0);

  useEffect(() => {
    if (impressions > 0) {
      setEcpmValue((revenue / impressions) * 1000);
    }
  }, [revenue, impressions]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem('local_stats_bot', JSON.stringify({
        revenue,
        impressions,
        clicks,
        updatedAt: Date.now(),
  ...
  // ===== COUNTDOWN: quando ciclo completa, inicia countdown de 1h =====
  useEffect(() => {
    ...
    if (cycleCompleted && secondsUntilReset > 0) {
      countdownIntervalRef.current = setInterval(() => {
        setSecondsUntilReset(prev => {
          if (prev <= 1) {
            // Reset expirou! Limpar tudo para novo ciclo
            ...
            // Limpar countdown salvo
            try { localStorage.removeItem('countdown_state'); } catch {}
            return 0;
          }
          // Salvar countdown no localStorage para persistir entre reloads
          try {
  ...
  }, [cycleCompleted, secondsUntilReset]);

  // Restaurar countdown do localStorage ao carregar
  useEffect(() => {
    try {
      const saved = localStorage.getItem('countdown_state');
      if (saved) {
        const state = JSON.parse(saved);
  ...
  useEffect(() => {
    ...
    if (typeof (window as any)[SDK_GLOBAL] === 'function') {
      setSdkReady(true);
      return;
    }
    ...
    script.setAttribute('data-sdk', SDK_GLOBAL);
    script.setAttribute('data-game-type', 'roulette');
    script.async = true;
    ...
    script.onload = () => {
      ...
      const interval = setInterval(() => {
        checks++;
        if ((window as any)[SDK_GLOBAL]) {
          clearInterval(interval);
          setSdkReady(true);
          console.log('[SDK] libtl.com/sdk.js carregado');
        } else if (checks > 30) {
          clearInterval(interval);
          console.warn('[SDK] Timeout');
        }
      }, 500);
    };

    script.onerror = () => {
      console.error('[SDK] Erro ao carregar');
    };
  ...
  const addLog = useCallback((msg: string) => {
    ...
    console.log(`[MONETAG] ${msg}`);
  }, []);



  // ===== AUTOMAÇÃO PRINCIPAL (SEM IA, RANDOM) =====
  const startAutomation = useCallback(() => {
    ...
    addLog(`[RANDOM] Timings gerados: delay ${timings.delayBetweenCyclesMin}-${timings.delayBetweenCyclesMax}ms, view ${Math.round(timings.viewTimeBase/1000)}s±${Math.round(timings.viewTimeVariance/1000)}s`);
    addLog(`[RANDOM] Cliques programados nos ciclos: #${timings.clickCycles.join(', #')}`);
    setStatus("Rodando...");
    ...
    const runLoop = async () => {
      ...
      while (!abortRef.current) {
        if (totalImpressions >= MAX_IMPRESSIONS && totalClicks >= MAX_CLICKS) {
          ...
          // Iniciar countdown de 1 hora
          setCycleCompleted(true);
          setSecondsUntilReset(RESET_DURATION_SECONDS);
          try {
            localStorage.setItem('countdown_state', JSON.stringify({
              secondsRemaining: RESET_DURATION_SECONDS,
              savedAt: Date.now(),
            }));
          } catch {}

          // Notificar IFTTT se configurado
          if (iftttWebhook) {
            try {
              fetch(iftttWebhook, {
                method: 'POST',
                ...
                body: JSON.stringify({
                  value1: `Metas atingidas! ${totalImpressions} imp, ${totalClicks} clicks`,
                  value2: config.ymid,
                  value3: `$${revenue.toFixed(4)}`,
                }),
              }).catch(() => {});
              addLog("[IFTTT] Notificação enviada!");
            } catch {}
          }
        ...
        // Chance de pausa extra (simulando distração)
        ...
        // Se impressões já completas e faltam cliques, delay mais curto
        if (totalImpressions >= MAX_IMPRESSIONS && totalClicks < MAX_CLICKS) {
        ...
        // === CLIQUE HUMANIZADO (aleatório nos ciclos programados) ===
        const shouldClick =
          clickUrl &&
        ...
        if (shouldClick) {
          ...
          const landingTime = 30000 + Math.random() * 30000; // 30-60s na landing
  ...
  }, [running, addLog, sdkReady, impressions, clicks, revenue, iftttWebhook, cycleCompleted, secondsUntilReset]);
          ...
          <div className="mx-4 mt-4 bg-white/[0.04] backdrop-blur-xl rounded-xl p-4 border border-white/10">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <span className="text-white/90 text-sm font-medium">
                {cycleCompleted ? formatTimeRemaining(secondsUntilReset) : 'A cada 1 hora'}
              </span>
            </div>
          </div>
        </div>
